        ...
        <>
            <div>
                {sector && <main className="flex flex-col items-center justify-between w-full h-dvh">
                    ...
                    <div>
                        {sector.images[0] && <svg width={800} height={600} viewBox="0 0 800 600" className="object-fit max-w-[800px] w-[100%] bg-neutral-900" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div className="w-full overflow-y-scroll">
                        {sector.boulders.map((boulder, i) => {
                            return <div className={`h-20 ${i % 2 === 0 ? "bg-neutral-200" : "bg-white"}`} key={i} onClick={() => setSelectedPath(boulder.id)}><p className={`w-full h-full p-3 ${selectedPath === boulder.id ? "bg-red-300" : ""}`}>{boulder.name}</p></div>
                        })}
                    </div>
                </main>}
            </div>
